...
_LIBZUTIL_H int zpool_disk_wait(int);
...
int for_each_leaf_vdev_macro_helper_func(void *zhp_data, nvlist_t *nv,
    void *data);
/*
 ...
 * 1000 vdevs = 82ms
 *
 * The NULL at the end of the for() loop gets around a "comma or semicolon
 * followed by non-blank" checkstyle error.
 */
#define	__FOR_EACH_VDEV(__zhp, __nv, __func) { \
	...
	for (nvlist_t *__root_nv = __nv, *__state = (nvlist_t *)0; \
		for_each_vdev_cb(&__state, __root_nv, __func, &__nv) == 1; NULL)
