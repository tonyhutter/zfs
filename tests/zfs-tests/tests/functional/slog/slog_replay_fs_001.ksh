...
#
log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/direct \
...
log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/payload \
    oflag=sync bs=1k count=8
typeset checksum=$(xxh128digest /$TESTPOOL/$TESTFS/payload)

# TX_WRITE (small file with ordering)
log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/small_file \
...
# TX_WRITE (large file)
log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/large \
...
log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/link_and_unlink \
    oflag=sync bs=128k count=8
...
if ! is_linux ; then
	...
	# TX_RENAME_EXCHANGE
	log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/xchg-a bs=1k count=1
	log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/xchg-b bs=1k count=1
	log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/xchg-c bs=1k count=1
	log_must dd if=$RANDPIPE of=/$TESTPOOL/$TESTFS/xchg-d bs=1k count=1
