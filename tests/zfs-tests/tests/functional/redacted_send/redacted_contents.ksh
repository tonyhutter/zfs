...
log_must dd if=$RANDPIPE of=$clone_mnt/f1 conv=notrunc seek=2 count=1 bs=32k
log_must zfs snapshot $clone@snap1
...
log_must dd if=$RANDPIPE of=$clone_mnt/f2 bs=1024k count=3 conv=notrunc
log_must zfs snapshot $clone@snap1
log_must zfs clone $sendfs@snap $clone/new
typeset mntpnt="$(get_prop mountpoint $clone/new)"
log_must dd if=$RANDPIPE of=$mntpnt/f2 bs=1024k seek=1 count=3 \
    conv=notrunc
...
log_must dd if=$RANDPIPE of=/$POOL/f1 bs=128k count=4
log_must zfs snapshot $POOL@snap
log_must zfs clone $POOL@snap $POOL/clone
log_must dd if=$RANDPIPE of=/$POOL/clone/f1 bs=128k count=1 conv=notrunc
