...
log_mustnot dd if=/$TESTPOOL/4k/4k_file1 of=/dev/null bs=4k count=10

# Try to read blocks 0 and blocks 4-5 and 6-7 on '4k_file2' to create multiple
# ranges.  The read should fail.
log_mustnot dd if=/$TESTPOOL/4k/4k_file2 of=/dev/null bs=4k count=1 skip=0
log_mustnot dd if=/$TESTPOOL/4k/4k_file2 of=/dev/null bs=4k count=2 skip=4
log_mustnot dd if=/$TESTPOOL/4k/4k_file2 of=/dev/null bs=4k count=2 skip=6

# Try to read the 2nd megabyte of '1m_file'
log_mustnot dd if=/$TESTPOOL/1m/1m_file of=/dev/null bs=1M skip=1 count=1
...
# it will.
log_mustnot dd if=${ZVOL_DEVDIR}/$TESTPOOL/$TESTVOL of=/dev/null bs=4k count=1 skip=1000
log_mustnot dd if=${ZVOL_DEVDIR}/$TESTPOOL/$TESTVOL of=/dev/null bs=4k count=3 skip=1100
