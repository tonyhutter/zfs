...
for i in 1 2 3; do
	...
	log_must randomdd of=/$TESTPOOL/$TESTFS1/testfile.$i bs=1M \
	    count=$blocks
	sync_pool
...
log_note "write data"
blocks=20
log_must randomdd of=/$TESTPOOL/$TESTFS1/testfile.3 bs=1M \
    count=$blocks
sync_pool
written=$(get_prop written $TESTPOOL/$TESTFS1)
...
log_must randomdd of=/$TESTPOOL/$TESTFS1/snap1.clone/testfile bs=1M \
    count=40
sync_pool
...
for ds in $datasets; do
	...
	[[ $writtenat -ne 0 ]] && \
	    log_fail "Unexpected written@ value"
	log_must randomdd of=/$ds/testfile bs=1M count=$blocks
	sync_pool
	writtenat=$(get_prop written@now $ds)
...
for ds in $datasets; do
	...
	log_must randomdd of=/$ds/testfile bs=1M \
	    count=$blocks
	sync_pool
done
