...
dd if=$RANDPIPE of=$TESTDIR/file_8k bs=1024k count=$MEGS oflag=sync \
    conv=notrunc >/dev/null 2>&1 || log_fail "dd into $TESTDIR/file failed."
log_must zfs set copies=3 $origin
dd if=$RANDPIPE of=$TESTDIR/file_8k_copies bs=1024k count=$MEGS oflag=sync \
    conv=notrunc >/dev/null 2>&1 || log_fail "dd into $TESTDIR/file failed."

log_must zfs set copies=1 $origin
log_must zfs set recordsize=128k $origin
dd if=$RANDPIPE of=$TESTDIR/file_128k bs=1024k count=$MEGS oflag=sync \
    conv=notrunc >/dev/null 2>&1 || log_fail "dd into $TESTDIR/file failed."
log_must zfs set copies=3 $origin
dd if=$RANDPIPE of=$TESTDIR/file_128k_copies bs=1024k \
