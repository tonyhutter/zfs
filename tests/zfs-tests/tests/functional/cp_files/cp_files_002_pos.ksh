...
for mode in "never" "auto" "always"; do
	...
	log_must dd if=$RANDPIPE of=$SRC_FILE bs=$RECORDSIZE count=$SRC_SIZE
	...
	sync_pool $TESTPOOL
	log_must dd if=$RANDPIPE of=$SRC_FILE bs=$RECORDSIZE seek=$SRC_SIZE \
	...
	sync_pool $TESTPOOL
	log_must dd if=$RANDPIPE of=$SRC_FILE bs=$((RECORDSIZE / 2)) \
