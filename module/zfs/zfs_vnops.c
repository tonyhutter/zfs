 ...
 *
 * Linux-only: relies on struct kiocb and ki_complete().
 */
 ...
 *
 * Linux-only: relies on struct kiocb, ki_complete(), -EIOCBQUEUED,
 * and the Linux VFS async I/O infrastructure.
 */
int
...
static void zfs_clear_setid_bits_if_necessary(zfsvfs_t *, znode_t *, cred_t *,
    uint64_t *, dmu_tx_t *);
...
/*
 * Async Direct I/O write.  Pins pages, acquires locks, creates a
 * transaction, and dispatches writes via dmu_write_abd_async().
 * The kworker returns 0 immediately; the caller returns -EIOCBQUEUED.
 *
...
/*
 * Module init: create dedicated caches and taskq for async I/O.
...
/*
 * Module fini: destroy caches and taskq created by zfs_async_dio_init().
