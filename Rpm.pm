...
#		or die "Unpacking of '".$this->filename."' failed";
	...
	$this->do("rpm2cpio '".$this->filename."' | (cd $workdir; $decomp cpio --extract --make-directories --no-absolute-filenames --preserve-modification-time | tee /tmp/out) 2>&1 | tee /tmp/out2 ")
		or die "Unpacking of '".$this->filename."' failed: ".$!;
