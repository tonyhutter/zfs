...
function generate() {
  ...
  # vm${N}log.txt   -> output of ssh/tail - used for merged summary
  # vm${N}/build-stderr.txt  -> used once
  ####################################################################
  # vm${N}/console.txt       -> all 3 used
  # vm${N}/dmesg-prerun.txt  -> used once
  # vm${N}/uname.txt         -> all 3 used
  # vm${N}/current/log       -> if not there, kernel panic loading
  # vm${N}/current/results   -> if not there, kernel panic testings
  # vm${N}/exitcode.txt      -> exitcode of testings partN
  ####################################################################
  ...
  for i in `seq 1 $VMs`; do
    ...
    dprerun="vm$i/dmesg-prerun.txt"
    test -s "$console" && showfile "$console" "vm$i: serial console output"
    test -s "$dprerun" && showfile "$dprerun" "vm$i: dmesg kernel output"
