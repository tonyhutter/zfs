...
# Usage:
#
#       qemu-4-build-vm.sh OS [--enable-debug][--dkms][--patch_level NUM]
...
# --patch_level NUM     Use a custom patch level number for packages.
...
function rpm_build_and_install() {
  ...
  if [ -n "$PATCH_LEVEL" ] ; then
    sed -iE 's/(Release:\s+)1/\1$PATCH_LEVEL/g' META
  fi
